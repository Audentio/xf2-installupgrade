<?php

namespace ThemeHouse\InstallAndUpgrade\Provider;

use ThemeHouse\InstallAndUpgrade\Entity\AddOn;
use ThemeHouse\InstallAndUpgrade\Entity\Language;
use ThemeHouse\InstallAndUpgrade\Entity\Product;
use ThemeHouse\InstallAndUpgrade\Entity\Profile;
use ThemeHouse\InstallAndUpgrade\Entity\Style;
use XF\Util\File;

class DragonByte extends AbstractHandler
{
	/** @var string */
	protected $identifier = 'dragonbyte';
	
	/** @var string */
	protected $apiKey;
	
	/** @var string */
	protected $baseUrl = 'http://localhost/devboards/xf21/api/dbtech-ecommerce/';
	
	/** @var Profile */
	protected $Profile;
	
	public $isUnique = true;
	
	public function tryAuthentication(Profile $profile)
	{
		return true;
	}
	
	/**
	 * @param Profile $profile
	 */
	public function authenticate(Profile $profile)
	{
		$this->apiKey = $profile->options['api_key'];
	}
	
	/**
	 * @param null $contentType
	 * @param null $contentId
	 * @param null $dataEntity
	 * @return null|Profile|\XF\Mvc\Entity\Entity
	 */
	public function getProfile($contentType = null, $contentId = null, $dataEntity = null)
	{
		if (!$this->Profile)
		{
			$this->Profile = $this->finder('ThemeHouse\InstallAndUpgrade:Profile')
				->where('provider_id', '=', 'dragonbyte')
				->fetchOne()
			;
		}
		
		return $this->Profile;
	}
	
	/**
	 * @param Profile $profile
	 * @throws \XF\PrintableException
	 */
	public function getProductsFromProvider(Profile $profile)
	{
		$this->authenticate($profile);
		
		\XF::db()->delete('xf_th_installupgrade_product', 'profile_id = ?', $profile->profile_id);
		
		$installed = $this->finder('ThemeHouse\InstallAndUpgrade:AddOn')
			->where('addon_id', 'LIKE', 'DBTech/%')
			->fetch()
			->pluck(function(AddOn $addOn)
			{
				if (empty($addOn->extra['product_id']))
				{
					return null;
				}
				
				return [$addOn->addon_id, $addOn->extra['product_id']];
			}, false)
		;
		
		// Rather than a bunch of if statements, we'll just construct the product filter this way
		//	If no products match the platform filter, it'll just return an empty array anyway
		$version = explode('.', \XF::$version);
		$platform = 'xf' . $version[0] . $version[1];
		
		$categoryIds = in_array($this->app->request()->getIp(), ['::1', '127.0.0.1', '172.21.0.1']) ? [1,2] : [5];
		
		// Response should be an array representation of License entities
		$addOns = $this->_getApiResponse('purchased/', [
			'category_ids' => $categoryIds,
			'platforms' => [$platform]
		]);
		
		if (empty($addOns['products']) || !$profile->exists())
		{
			return;
		}
		
		foreach ($addOns['products'] as $addOn)
		{
			if (in_array($addOn['product_id'], $installed))
			{
				continue;
			}
			
			$product = $this->em->create('ThemeHouse\InstallAndUpgrade:Product');
			$product->bulkSet([
				'profile_id'   => $profile->profile_id,
				'product_id'   => $addOn['product_id'],
				'product_type' => 'addOn',
				'title'        => $addOn['full_title'],
				'description'  => $addOn['tagline'],
				'version'      => $addOn['LatestVersion']['version_string'],
				'extra'        => [
					'product_page' => $addOn['product_page_url'],
					'thumbnail'    => 'data:image/png;base64,',
				]
			]);
			if ($product->preSave())
			{
				$product->save();
			}
		}
	}
	
	/**
	 * @param AddOn $addOn
	 * @throws \XF\PrintableException
	 */
	public function checkAddOnForUpdates(AddOn $addOn)
	{
		try
		{
			$latestDownload = $this->_getLatestDownload($addOn->extra['product_id']);
		}
		catch (\XF\PrintableException $e)
		{
			return;
		}
		
		$version = $latestDownload['version_string'];
		
		$addOn->latest_version = $version;
		$addOn->update_available = $this->compareVersions($addOn->AddOn->version_string, $version);
		$addOn->save();
	}
	
	/**
	 * @param AddOn $addOn
	 *
	 * @return string
	 * @throws \XF\PrintableException
	 */
	public function downloadAddOn(AddOn $addOn)
	{
		$latestDownload = $this->_getLatestDownload($addOn->extra['product_id']);
		if (!$latestDownload['can_download'])
		{
			throw new \XF\PrintableException('Your license does not permit you to upgrade to this version. Please renew your license.');
		}
		
		return $this->_getZipFromApi('addOn', $addOn->extra['product_id'], $latestDownload['download_id']);
	}
	
	/**
	 * @param Product $product
	 *
	 * @return bool|string
	 * @throws \XF\PrintableException
	 */
	protected function _downloadProduct(Product $product)
	{
		$downloadableId = null;
		
		$downloads = $this->_getDownloads($product->product_id);
		foreach ($downloads as $download)
		{
			if ($download['can_download'])
			{
				$downloadableId = $download['download_id'];
				break;
			}
		}
		
		if (!$downloadableId)
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] No downloadable versions could be found.');
		}
		
		return $this->_getZipFromApi($product->product_type, $product->product_id, $downloadableId);
	}
	
	/**
	 * @param $endpointUrl
	 * @param array $params
	 *
	 * @return bool|mixed
	 * @throws \XF\PrintableException
	 */
	protected function _getApiResponse($endpointUrl, array $params = [])
	{
		$success = true;
		$error = null;
		$responseJson = [];
		
		$client = $this->app->http()->client();
		
		try
		{
			$endpointUrl = $this->baseUrl . $endpointUrl . '?' . http_build_query($params);
			$response = $client->get($endpointUrl, [
				'headers' => $this->_getApiHeaders()
			]);
			
			$responseJson = $response->json();
			
			if ($response->getStatusCode() !== 200)
			{
				$success = false;
				if (isset($responseJson['errors']))
				{
					$error = $responseJson['errors'][0]['message'];
				}
				else
				{
					$error = 'An unexpected error occurred.';
				}
			}
			else if (!is_array($responseJson))
			{
				$success = false;
				$error = 'Invalid API response: ' . var_export($responseJson, true);
			}
		}
		catch (\GuzzleHttp\Exception\ClientException $e)
		{
			$success = false;
			
			$response = $e->getResponse();
			$responseJson = $response->json();
			
			if (isset($responseJson['errors']))
			{
				$error = $responseJson['errors'][0]['message'];
			}
			else
			{
				$error = $e->getMessage();
			}
		}
		catch (\GuzzleHttp\Exception\RequestException $e)
		{
			$success = false;
			$error = $e->getMessage();
		}
		
		if (!$success)
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] API error: ' . $error);
		}
		
		return $responseJson;
	}
	
	/**
	 * @param $productId
	 *
	 * @return array
	 * @throws \XF\PrintableException
	 */
	protected function _getLatestDownload($productId)
	{
		$this->authenticate($this->getProfile());
		
		// Response should be an array representation of DownloadVersion entity
		$response = $this->_getApiResponse($productId . '/latest-version/', [
			'product_version' => 'xf2',
			'product_version_type' => 'full',
		]);
		
		if (!empty($response['errors']))
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] ' . $response['errors'][0]['message']);
		}
		
		if (empty($response['latestVersion']))
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] No downloadable versions could be found.');
		}
		
		return $response['latestVersion'];
	}
	
	/**
	 * @param $productId
	 *
	 * @return array
	 * @throws \XF\PrintableException
	 */
	protected function _getDownloads($productId)
	{
		$this->authenticate($this->getProfile());
		
		// Response should be an array representation of DownloadVersion entity
		$response = $this->_getApiResponse($productId . '/downloads/', [
			'product_version' => 'xf2',
			'product_version_type' => 'full',
		]);
		
		if (!empty($response['errors']))
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] ' . $response['errors'][0]['message']);
		}
		
		if (empty($response['downloads']))
		{
			throw new \XF\PrintableException('[DragonByte Install & Upgrade] No downloadable versions could be found.');
		}
		
		return $response['downloads'];
	}
	
	/**
	 * @param $productType
	 * @param $productId
	 * @param $download
	 *
	 * @return string
	 * @throws \XF\PrintableException
	 */
	protected function _getZipFromApi($productType, $productId, $download)
	{
		$this->authenticate($this->getProfile());
		
		$tempPath = File::getNamedTempFile('dbtech-' . $productType . \XF::$time . '.zip');
		
		try
		{
			$endpointUrl = $this->baseUrl . 'downloads/' . $download . '/download?' . http_build_query([
				'product_version' => 'xf2',
				'product_version_type' => 'full',
			]);
			if (in_array($this->app->request()->getIp(), ['::1', '127.0.0.1', '172.21.0.1']))
			{
				$response = \XF::app()->http()->reader()->get($endpointUrl, [], null, [
					'headers' => $this->_getApiHeaders()
				], $error);
			}
			else
			{
				$response = \XF::app()->http()->reader()->getUntrusted($endpointUrl, [], null, [
					'headers' => $this->_getApiHeaders()
				], $error);
			}
			if (!$response)
			{
				throw new \XF\PrintableException('[DragonByte Install & Upgrade] Unable to download zip from DragonByte: ' . $error);
			}
			
			switch ($response->getStatusCode())
			{
				case 402:
					throw new \XF\PrintableException('Your license does not permit you to upgrade to this version. Please renew your license.');
					break;
				
				case 403:
					throw new \XF\PrintableException('Invalid API key.');
					break;
				
				case 200:
					// Success
					break;
				
				default:
					throw new \XF\PrintableException('[DragonByte Install & Upgrade] Unable to download zip from DragonByte.');
					break;
			}
			
			$stream = $response->getBody();
			
			do
			{
				file_put_contents($tempPath, $stream->read(1024), FILE_APPEND);
			}
			while (!$stream->eof());
		}
		
		catch (\GuzzleHttp\Exception\ClientException $e)
		{
			$response = $e->getResponse();
			switch ($response->getStatusCode())
			{
				case 402:
					throw new \XF\PrintableException('Your license does not permit you to upgrade to this version. Please renew your license.');
					break;
				
				case 403:
					throw new \XF\PrintableException('Invalid API key.');
					break;
				
				default:
					throw new \XF\PrintableException('[DragonByte Install & Upgrade] Unable to download zip from DragonByte.');
					break;
			}
		}
		catch (\GuzzleHttp\Exception\RequestException $e)
		{
			throw new \XF\PrintableException(
				'[DragonByte Install & Upgrade] Unable to download zip from DragonByte: ',
				$e->getCode(),
				$e
			);
		}
		
		$abstractPath = sprintf('internal-data://install-upgrade/%s/dbtech-%d-%d.zip',
			$productType, $productId, \XF::$time);
		File::copyFileToAbstractedPath($tempPath, $abstractPath);
		
		return $abstractPath;
	}
	
	/**
	 * @return array
	 */
	protected function _getApiHeaders()
	{
		return [
			'XF-Api-Key' => $this->apiKey,
			'X-DragonByte-BoardUrl' => \XF::options()->boardUrl,
			'X-DragonByte-HttpHost' => $this->app->request()->getServer('HTTP_HOST'),
			'X-DragonByte-Software' => 'XenForo',
			'X-DragonByte-SoftwareVersion' => \XF::$versionId
		];
	}
	
	/**
	 * @param Style $style
	 * @return null|string
	 */
	public function downloadStyle(Style $style)
	{
		return true;
	}
	
	/**
	 * @param Style $style
	 */
	public function checkStyleForUpdates(Style $style)
	{
	}
	
	/**
	 * @param Language $language
	 * @return null|string
	 */
	public function downloadLanguage(Language $language)
	{
		return true;
	}
	
	/**
	 * @param Language $language
	 */
	public function checkLanguageForUpdates(Language $language)
	{
	}
	
	/**
	 * @param $url
	 * @return null|Profile|\XF\Mvc\Entity\Entity
	 */
	public function getProfileForUrl($url)
	{
		return $this->getProfile();
	}
	
	/**
	 * @param $url
	 * @param $type
	 * @return bool
	 */
	public function createProductFromUrl($url, $type)
	{
		return false;
	}
	
	/**
	 * @param string $class
	 *
	 * @return \XF\Service\AbstractService
	 */
	protected function service($class)
	{
		return call_user_func_array([$this->app, 'service'], func_get_args());
	}
}